        ...
        foreach ($posList as $pos => $type) {
            ...
            if ($arg_number_length = strspn($sql, '0123456789', $new_pos_back)) {
                ...
                if (!array_key_exists($arg_number, $args_all))
                    return $this->nonSQLError("Non existent argument reference (arg $arg_number): $sql");
                ...
                if (count($args_all) != 1 || !is_array($args_all[0]))
                    return $this->nonSQLError("If you use named parameters, the second argument must be an array of parameters");
    ...
    public function parseQueryParams()
    {
        ...
        foreach ($chunkyQuery as $chunk) {
            ...
            if ($type == 's')
                ...
                return $this->nonSQLError("Badly formatted SQL query: Invalid MeekroDB param $type");
...
class WhereClause
{
    ...
    function __construct($type)
    {
        ...
        if ($type !== 'or' && $type !== 'and')
            return DB::nonSQLError('you must use either WhereClause(and) or WhereClause(or)');
        $this->type = $type;
    }
